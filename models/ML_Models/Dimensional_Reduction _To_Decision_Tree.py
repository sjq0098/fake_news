...
# 划分训练集和验证集
X_tr_pca, X_val_pca, y_tr, y_val = train_test_split(X_train_pca, X_label, test_size=0.2, random_state=42)
...
model_name = "Dimensional_Reduction _To_DecisionTree"  # 可根据实际情况修改
# 生成分类报告
...
train_sizes, train_scores, val_scores = learning_curve(
    model, X_tr_pca, y_tr, train_sizes=np.linspace(0.1, 0.5, 5), cv=3, scoring='accuracy', n_jobs=1, random_state=42
)
...
plt.title(f"{model_name}_learning_curve")
