...
# 创建一个字典来存储每个 alpha 的交叉验证结果
results = {}

for alpha in alpha_values:
    nb = MultinomialNB(alpha=alpha)
    scores = cross_val_score(nb, X_train, X_label, cv=5)  # 使用5折交叉验证
    results[alpha] = scores.mean()  # 保存每个 alpha 的平均准确率
...
model_name = "MultinomialNB"  # 可根据实际情况修改
# 生成分类报告
